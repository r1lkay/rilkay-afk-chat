...
public class Main extends JavaPlugin implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        // /rafk reload
        if (command.getName().equalsIgnoreCase("rafk")) {
            if (args.length > 0 && args[0].equalsIgnoreCase("reload")) {
                if (!sender.hasPermission("rilkayafk.reload")) {
                    sender.sendMessage(miniMessage.deserialize("<#f5dda2>You can't use this command!"));
                    return true;
                }
                reloadConfig();
        ...
        if (command.getName().equalsIgnoreCase("afk")) {
            ...
            if (!player.hasPermission("rilkayafk.afk")) {
                player.sendMessage(miniMessage.deserialize("<#f5dda2>You can't use this command!"));
                return true;
            }
